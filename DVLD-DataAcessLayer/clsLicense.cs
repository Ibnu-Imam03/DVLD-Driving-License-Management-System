...
namespace DVLD_DataAccess
{
    public class clsLicenseData
    {
        ...
        public static bool IsLocalDrivingLicenseApplicationExist(int PersonID,int LicenseClassID)
        {
            SqlConnection connection =new SqlConnection(PeopeleDatasettings.ConnectionString);
            bool isFound = false;
            string query = @"SELECT 1 FROM LocalDrivingLicenseApplications AS LDLA INNER JOIN Applications AS A ON LDLA.ApplicationID = A.ApplicationID
                               WHERE A.ApplicationPersonID = @PersonID AND LDLA.LicenseClassID = @LicenseClassID AND A.ApplicationStatus = 1;";
            ...
            {
                connection.Open();
                object raesult = command.ExecuteScalar();
                if (raesult != null)
                    isFound = true;
            }
            catch
